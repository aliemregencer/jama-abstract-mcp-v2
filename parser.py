    ...
    for icon_file, keywords in ICON_MAP.items():
        for keyword in keywords:
            if keyword in search_text:
                print(
                    f"'{keyword}' anahtar kelimesi bulundu. '{icon_file}' ikonu seçildi."
                )
...
def parse_jama_article(url):
    """
    Verilen JAMA Network Open makale URL'sini Selenium ile açar,
    içeriğini alır ve yapılandırılmış bir dictionary olarak döndürür.
    """

    html_content = None

    # YENİ ve GÜNCELLENMİŞ Selenium ayarları
    chrome_options = Options()
    chrome_options.add_argument("--headless")  # Tarayıcıyı arayüz olmadan çalıştırır
    chrome_options.add_argument(
        "--no-sandbox"
    )  # Docker gibi konteyner ortamları için GEREKLİ bir ayardır
    chrome_options.add_argument(
        "--disable-dev-shm-usage"
    )  # Bellek (RAM) ile ilgili olası hataları önler
    chrome_options.add_argument(
        "--disable-gpu"
    )  # GPU kullanımını devre dışı bırakır, headless için önerilir
    chrome_options.add_argument(
        "--window-size=1920,1080"
    )  # Sayfanın düzgün render edilmesi için sanal bir ekran boyutu belirler
        ...
        pprint(
            {
                ...
                if k not in ["full_text", "references"]
            }
        )
